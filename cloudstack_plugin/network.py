...
def create(ctx, **kwargs):
    ...
    if not _network_exists(cloud_driver, network_name):
        ctx.logger.info('creating network: {0}'
            .format(network_name))
        nets = cloud_driver.ex_create_network(name=network_name, display_text=network['description'], location=location, network_offering=network_offering)
       ...
       #         raise RuntimeError('You must specify start_port for a portforward rule')
       #     end_port = rule.get('end_port', None)
       #     _add_ingress_rule(ctx, cloud_driver, network_name=network_name,
       #               start_port=start_port,
       #               end_port=end_port,
       #               cidr_list=cidr,
       #               protocol=protocol)
    else:
        ctx.logger.info('using existing management network {0}'.format(network_name))
...
def delete(ctx, **kwargs):
    try:
        cloud_driver = get_cloud_driver(ctx)
        cloud_driver.ex_delete_network(ctx.runtime_properties['external_id'])
    except:
        ctx.logger.warn(
...
def _add_ingress_rule(ctx, cloud_driver, node_name, protocol, cidr_list, start_port,
              end_port=None):
